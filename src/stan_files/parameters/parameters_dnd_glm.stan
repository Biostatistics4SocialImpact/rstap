  ...
  vector[prior_dist_for_stap == 7 ? sum(num_normals) : num_bar] z_beta_bar;
  ...
  real<lower=0,upper=max_distance> shape_s[num_s_wei > 0 ? num_s_wei : 0 ]; // shape for spatial weight function
  real<lower=0,upper=max_distance> shape_t[num_t_wei > 0 ? num_t_wei : 0 ]; // shape for temporal weight function
