  ...
  vector<lower=0>[Q_s > 0 ? M : 0] dists_crs[Q_s + Q_st]; //distance crs matrix 
  vector<lower=0>[Q_t > 0 ? M : 0] times_crs[Q_t + Q_st]; // time crs matrix
  int<lower=0> w[Q,2]; // spatial-temporal weight functions 0 = no component; 1 = erf; 2 = cerf; 3 = exp; 4 = cexp;
  // Meta data
  int u_s[Q_s > 0 ? N: 0, Q_s > 0 ? ((Q_s + Q_st) * 2) : 0]; //index holder array for distances
  int u_t[Q_t >0 ? N : 0, Q_t > 0 ? ((Q_s + Q_st) * 2) : 0]; // index holder array for times
