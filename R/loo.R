...
#'   \link{stapreg-objects}.
#'
#'   For \code{loo_model_weights}, \code{x} should be a "stapreg_list"
#'   object, which is a list of fitted model objects created by
#'   \code{\link{stapreg_list}}.
#'
...
#'   package).
#'
#' @section Approximate LOO CV: The \code{loo} method for stapreg objects
...
#'   The \code{k_threshold} argument to the \code{loo} method for \pkg{rstap}
...
#'   \item \code{\link{log_lik.stapreg}} to directly access the pointwise
...
#'   \link[=stapreg-objects]{stapreg} objects and the indices of the omitted
  ...
  for (k in 1:K) {
    ...
    lppds[[k]] <-
      log_lik.stapreg(
        ...
        stanmat = as.matrix.stapreg(fit_k)
      )
    if (save_fits) {
...
compare_models <- function(..., loos = list(), detail = FALSE) {
  ...
  structure(
    comp,
    class = c("compare_rstap_loos", class(comp)),
...
#'   compute model weights for a \code{"stapreg_list"} object, which is a list
#'   of fitted model objects made with \code{\link{stapreg_list}}. The end of
...
#'   package.
#'
loo_model_weights.stapreg_list <-
  function(x,
           ...,
           cores = getOption("mc.cores", 1),
           k_threshold = NULL) {
    ...
    for (j in seq_along(x)) {
      loo_list[[j]] <-
        loo.stapreg(x[[j]], cores = cores, k_threshold = k_threshold)
    }
  ...
  for (j in 1:J) {
    ...
    lls[[j]] <-
      log_lik.stapreg(
        ...
        newx = get_x(x)[omitted, , drop = FALSE],
        newz = x$z[omitted, , drop = FALSE], # NULL other than for some stan_betareg models
        stanmat = as.matrix.stapreg(fit_j)
      )
  }
